...
// need these for testing & debugging only
...
public class DiscriminantAnalysisGUI extends JPanel
	...
	protected final static Logger logger = 
		Logger.getLogger(DiscriminantAnalysisGUI.class.getPackage().getName());
	...
	/**
	 * main method is used exclusively for testing and debugging
	 * purposes only
	 * @param args
	 */
	public static void main(String[] args) {
		...
		//URL testFileName = daGui.getClass().getResource("resources/electoral_divisions_cu_cso_2006.shp");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == this.categoryCombo) {
	...
	public void dataSetChanged(DataSetEvent e) {
		...
		for (int i = 0;  i < numVars; i++){
			...
			if ( dTypes[i] == DataSetForApps.TYPE_INTEGER ) {
				...
				categoryIndexMap.put(
						Integer.valueOf(++categoryComboIndex), 
	...
	private void classify() {
		...
		if ( categoryIndex >= 0 && indVarIndices.length > 0 ) {
			...
			logger.info("Classification Category : [" + 
					dataSet.getNumericArrayName(categoryIndex) + "], numeric index : [" + 
					categoryIndex + "]");
			logger.info("Independent Variables:");
			for (int i=0; i < indVarIndices.length; i++) {
				logger.info("[" + 
							dataSet.getNumericArrayName(indVarIndices[i]) + 
							"], numeric index : [" + indVarIndices[i] + "]");
			}
			
			logger.info("Data set contains [" + 
						String.valueOf(dataSet.getRowCount()) +
						"] observations");
			logger.info("Dataset contains [" + String.valueOf(dataSet.getColumnCount()) + "] fields");
			...
			//Object[] dataSetChanged = {fNames, da.getClassification(), da.getClassified(), da.getMahalanobisDistance2(0), dataSet.getShapeData()};
		
			//DataSetForApps dataSetChanged = new DataSetForApps(dataSet.getDataSetFull());
			//dataSetChanged.addColumn("Classified", da.getClassified());
			//for (int i = 0; i < numClasses; i++) {
				//dataSetChanged.addColumn("MhDist2_" + String.valueOf(i), da.getMahalanobisDistance2(i));
				//dataSetChanged.addColumn("PostProb_" + String.valueOf(i), da.getPosteriorProbabilities(i));
			//}
			...
			logger.info("Category Index is [" + ((categoryIndex < 0) ? "not set" : "set") + "]");
			logger.info("Independent Variables are [" + ((indVarIndices.length == 0) ? "not set" : "set") + "]" );
		}
	}
	 ...
	 * Consistent. 
	 * Also check to see if the categoryIndex is contained in the 
	...
	private void checkIndVarIndices() {
		...
		if ( categoryIndex >= 0 && indVarIndices.length > 0  ) {
			...
			if (found >= 0) {
				...
				this.indVarIndices = indVarIndicesNew;
			}
		}
	}
