...
//	Factory<Graph<Number,Number>> graphFactory;
	...
	public void start() {
		...
		try {
			g = rm
			.readMatrixtoGraph("C:\\Users\\weiluo\\Desktop\\Work with Chanda\\PoliticalBorderNileBasinReorderCorrected.txt");
	...
	private void setUpView() throws IOException {
        ...
        /*PajekNetReader<Graph<Integer, Number>, Integer,Number> pnr = 
            new PajekNetReader<Graph<Integer, Number>, Integer,Number>(vertexFactory, edgeFactory);
        
        final Graph<Integer,Number> graph = new SparseMultigraph<Integer, Number>();
        ...
        layout = 
        	new AggregateLayout<Integer,Number>(new FRLayout<Integer,Number>(g));
...
//		edgeBetweennessSlider.setBorder(BorderFactory.createLineBorder(Color.black));
	...
	public void clusterAndRecolor(AggregateLayout<Integer,Number> layout,
		...
		Graph<Integer,Number> g = layout.getGraph();
        layout.removeAll();

		EdgeBetweennessClusterer<Integer,Number> clusterer =
			new EdgeBetweennessClusterer<Integer,Number>(numEdgesToRemove);
	...
	private void groupCluster(AggregateLayout<Integer,Number> layout, Set<Integer> vertices) {
		if(vertices.size() < layout.getGraph().getVertexCount()) {
			Point2D center = layout.transform(vertices.iterator().next());
			Graph<Integer,Number> subGraph = SparseMultigraph.<Integer,Number>getFactory().create();
			for(Integer v : vertices) {
				subGraph.addVertex(v);
			}
			Layout<Integer,Number> subLayout = 
				new CircleLayout<Integer,Number>(subGraph);
