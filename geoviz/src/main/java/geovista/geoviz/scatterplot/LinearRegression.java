...
public class LinearRegression implements Regression {
	...
	public double getRSquare(double[] dataX, double[] dataY) {
		return DescriptiveStatistics.correlationCoefficientIgnoreNaN(dataX,
				dataY, false);
		// // the goodness of fit
		// double rSqr = 0;
		...
		// double SYY = 0;
		// double RSS = 0;
		// regressionElements = getRegression(dataX, dataY);
		// for (double element : dataY) {
		// sumY = sumY + element;
		// }
		// meanY = sumY / dataY.length;
		...
		// .pow(
		// dataY[i]
		// - (regressionElements[0] * dataX[i] + regressionElements[1]),
		// 2);
		// }
		// if (SYY != 0) {
		// rSqr = 1 - RSS / SYY;
