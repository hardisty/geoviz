...
public class TableBrowser extends JPanel implements ChangeListener,
		Serializable, SelectionListener, DataSetListener {

	private final Container contentPane = this;
	...
	public void selectionChanged(SelectionEvent e) {
		setSelectedIndex(e.getSelection());
	}

	public SelectionEvent getSelectionEvent() {
		return new SelectionEvent(this, new int[] { 2, 3, 4 });
	}
	...
	public void setDataSet(DataSetForApps dataSet) {
		...
		for (int column = 0; column < attributeNames.length; column++) {
			Object datum = dataSet.getNamedArrays()[column];// XXX is this the
			// right accessor?
	...
	/*
	 * public TableBrowser(int column, int numOfRows) { tableModel = new
	 * MxmTableModel(column,numOfRows); settableProperties(); // initailized the
	 * GUI buildGUI(); }
	 */
	...
	public void setSelectedIndex(int[] selectedRows) {
		...
		for (int i = 0; i < len; i++) {
			...
			currentTable.addRowSelectionInterval(selectedRows[i],
					selectedRows[i]);
	...
	private void settableProperties() {
		...
		if (model == 1) {
			MxmTableCellRenderer renderer = new MxmTableCellRenderer(
					JLabel.LEFT);
	...
	private void buildMenue() {
		...
		Action[] actions = { new NewAction(), new OpenAction(),
				new PrintAction(), new CutAction(), new CopyAction(),
				new PasteAction(), new ExitAction() };
		...
		Action[] actions1 = { new PropertiesAction() };
		...
		/*
		 * try{ iconIDL=new ImageIcon(new URL(codebaseURL,"idl.gif"));
		 * }catch(Exception ed) { System.out.println("can not find the image"); }
		 */
	...
	public void resizeRowOfCurrentTable(int amplify, int shrink) {
		...
		currentTable.setRowHeight(currentTable.getRowHeight() * amplify
				/ shrink);
	...
	public void resizeColumnOfCurrentTable(int amplify, int shrink) {
		...
		for (int i = 0; i < tcm.getColumnCount(); i++) {
			...
			w = column.getWidth();
			column.setPreferredWidth((w * amplify / shrink));
			column.setWidth((w * amplify / shrink));
		}
	...
	public void setTableMode() {
		JCheckBox[] checkBox = getMyControl().getTableMode();
	...
	private void ScrollTable(int row, int col, int amplify, int shrink) {
		JScrollPane myScrollPane = (JScrollPane) SwingUtilities
				.getAncestorOfClass(JScrollPane.class, currentTable);
