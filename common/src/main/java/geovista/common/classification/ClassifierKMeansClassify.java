...
public class ClassifierKMeansClassify {
  ...
  private int[] cluster; // each element indicates the class number of the corresponding state.
  private double[][] clustermean; // each row is a mean vector of a class.
  private int[] selectedAttIdx; //the indices of attributes considered in this classification.
  private String[] attributesDisplay;
  private double[][] dataArray; // array for classification, each row is a data vector of an observation, each collum is an attribute vector.
  ...
  {
          // loop to classify
          double[] obsWithConsideredAtt = new double [this.selectedAttIdx.length];//pick data within considered attributes for each observation.
          ...
          for (int j=0;j<dataArray.length;j++) // every vector
          {
            ...
            double distance = 0.0;
            cluster[j]=0;
            //copy those considered attributes value to array for distance calculation
            for ( int k=0;k<this.selectedAttIdx.length;k++){
              obsWithConsideredAtt[k] = dataArray[j][this.selectedAttIdx[k]];
            }

            for (int k=0;k<clustermean.length;k++) // every cluster mean
            {
  ...
  public static double getDistance(double[] a, double[] b)
{
      ...
      public static double distance2(double[] a, double[] b) // square of distance
      {
              double c=0.0;
