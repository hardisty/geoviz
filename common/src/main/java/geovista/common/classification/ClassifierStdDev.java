...
public class ClassifierStdDev implements DescribedClassifier, BoundaryClassifier  {
  ...
  protected final static Logger logger = Logger.getLogger(ClassifierStdDev.class.getName());
...
//        }
  ...
  public int[] classify(double[] data, int numClasses) {
    ...
    //if the number of classes is only 1, just classify all the data into one class
    if (numClasses == 1) {
    ...
    if (Math.floor(numClasses / 2) == (numClasses / 2)) {
      for (int i = 0; i < data.length; i++) {
        for (int j = 0; j < (numClasses / 2); j++) {
          //data less than the lower limit is classified into the first class
          if (data[i] <= (mean - (((double)numClasses - 2) / 2) * stdDev)) {
            classification[i] = 0;
          }

          //data greater than the upper limit is classified into the last class
          if (data[i] > (mean + (((double)numClasses - 2) / 2) * stdDev)) {
            classification[i] = numClasses-1;
          }
          //other data are classified as follows
          else {
            if ((data[i] > (mean - ((numClasses / 2) - j - 1) * stdDev)) &&
                  (data[i] <= (mean - ((numClasses / 2) - j - 2) * stdDev))) {
    ...
    //When 'numClasses' is odd, do the classification similarly with a little difference
    if (Math.floor(numClasses / 2) < (numClasses / 2)) {
      for (int i = 0; i < data.length; i++) {
        for (int j = 0; j <= ((numClasses + 1) / 2); j++) {
          ...
          //data between mean plus stdDev and mean minus stdDev is classified into the middle class
          ...
          //other data are classified as follows
            ...
            if (data[i] <= mean) {
              if (((data[i] > (mean -
                        (((numClasses + 1) / 2) - j - 1) * stdDev)) &&
                      (data[i] <= (mean - (((numClasses + 1) / 2) - j - 2)) * stdDev))) {
                classification[i] = j + 1;
              }
