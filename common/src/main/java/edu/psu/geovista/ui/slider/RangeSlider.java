 ...
 *
 * Java source file for the class GvRangeSlider
 *
 ...
 * $Author: xpdai $
 *
 * $Id: RangeSlider.java,v 1.2 2003/07/16 16:14:09 xpdai Exp $
 *
...
public class RangeSlider extends JPanel
	...
	protected final static Logger logger = Logger.getLogger(RangeSlider.class.getName());
	...
	public RangeSlider () {
		super();
		divideResults = false;
        //initialize(DEFAULT_NAME, DEFAULT_ORIENTATION, DEFAULT_NUMBER_OF_THUMBS,
     ...
     * @param		 int divisor
     */
	public RangeSlider (String attributeName, int orientationConstant, int numberOfThumbs,
			int sliderMinimum, int sliderMaximum, int majorTicks, int minorTicks,
    //int divisor) {
	double divisor) {
		super();
		divideResults = false;
		initialize(attributeName, orientationConstant, numberOfThumbs, sliderMinimum,
				sliderMaximum, majorTicks, minorTicks, divisor);
	}
	...
	double precision)              // 0.01
	{
		...
		initialize(attributeName, DEFAULT_ORIENTATION, DEFAULT_NUMBER_OF_THUMBS,
				(int)(sliderMinimum*divisor),               // 0
    ...
    // ============================ METHODS ========================================== //
	private void initialize (String attributeName, int orientationConstant, int numberOfThumbs,
			int sliderMinimum, int sliderMaximum, int majorTicks, int minorTicks, double divisor) {
		...
		rangeSlider.setPaintLabels(this.paintLabel);
        ...
        /*sliderNumberDisplayed = new JLabel[numberOfThumbs];
         for (int i = 0; i < numberOfThumbs; i++) {
         sliderNumberDisplayed[i] = new JLabel();
         sliderNumberDisplayed[i].setHorizontalAlignment(SwingConstants.CENTER);
         sliderNumberDisplayed[i].setForeground(fontColor);
         sliderNumberDisplayed[i].setText(Double.toString((double)rangeSlider.getValueAt(i)/divisor));
         //				+ " " + getUnits());
    ...
    /**
     * If labels are a value other than a double, this method converts and sets the
     * new labels in the corect format
     * @param divisor
     * @param sliderMinimum
     * @param sliderMaximum
     */
     ...
     * @param divisor
     */
	private void createSliderLabel (Hashtable hashLabels, int value, double divisor) {
		double dividedValue = (double)value/divisor;
        logger.finest("GvAS.createSliderLabel(), value = " + value + ", dividedValue = " + dividedValue);
	...
	class RangeSliderHandler
		...
		public void stateChanged (ChangeEvent ce) {
			...
			toolTip = " ";
			for (int i = 0; i < numberOfThumbs; i++) {
				double valueAdjusted = slider.getValueAt(i)/(double)divisor;
				try {
                    //sliderNumberDisplayed[i].setText("	" + Double.toString(valueAdjusted));
                    //+ " " + getUnits());
					if (i == rangeSlider.getCurrentThumbIndex()) {
						toolTip = Double.toString((double)rangeSlider.getValueAt(i)/divisor);
					}
                    //toolTip = toolTip + Double.toString((double)rangeSlider.getValueAt(i)/divisor) + " ";
					if (i == 0) {
						titleValues = Double.toString(valueAdjusted) + "           " + title +
								"           " + Double.toString((double)rangeSlider.getValueAt(1)/divisor);
					}
					else {
						titleValues = Double.toString((double)rangeSlider.getValueAt(0)/divisor)
								+ "           " + title + "           " + Double.toString(valueAdjusted);
					}
					rangeSlider.setBorder(BorderFactory.createTitledBorder(BorderFactory.createRaisedBevelBorder(),
	...
	public double[] getValueDoubles () {
		...
		for (int i = 0; i < numThumbs; i++) {
			if (divideResults)
				arrayReturnValues[i] = ((double)rangeSlider.getValueAt(i)/(double)divisor);
			else
				arrayReturnValues[i] = (double)rangeSlider.getValueAt(i);
		}
		return  arrayReturnValues;
	}
	...
	public void setTitle (String s) {
		...
		if (!this.title.equals(s) || title.equals(RangeSlider.DEFAULT_TITLE)) {
			...
			if (divideResults == false && this.rangesChanged == false) {
				...
				this.attChanged = true;
				this.rangesChanged = true;
				this.titleChanged = false;
                //repaint();
                //set the border name to include variable name and maximum and minimum numbers.
                //if ((rangeSlider != null)){
				titleValues = Double.toString((double)rangeSlider.getValueAt(0)/divisor)
						+ "           " + title + "           " + Double.toString((double)rangeSlider.getValueAt(1)/divisor);
				rangeSlider.setBorder(BorderFactory.createTitledBorder(BorderFactory.createRaisedBevelBorder(),
						titleValues, TitledBorder.CENTER, TitledBorder.ABOVE_TOP));
                //}else {
                //    titleValues = title;
	...
	public void setUnits (String s) {
		units = s;
		for (int i = 0; i < numberOfThumbs; i++) {
			double valueAdjusted = rangeSlider.getValueAt(i)/(double)divisor;
			try {
                //sliderNumberDisplayed[i].setText("	" + Double.toString(valueAdjusted));
                //+ " " + getUnits());
	...
	public void setMaxValue (double max) {
		int intmax = (int)(max*(double)divisor);
	...
	public void setMinValue (double min) {
		int intmin = (int)(min*(double)divisor);
	...
	public void setMinMaxValue (double[] minMax) {
		...
		if ((divideResults == false && this.attChanged == false)) {
			double precision = 1.0;
			divisor = 1.0/precision;
			divideResults = true;
			...
			if (this.titleChanged == false && this.title != null) {
				this.rangesChanged = true;
			}
			else
	...
	public void setRangeTitle (Object[] rangeTitle) {
		...
		if (title != (String)rangeTitle[0]) {
			...
			this.title = (String)rangeTitle[0];
			double precision = 1.0;
			divisor = 1.0/precision;
			totalLength = (ranges[1] - ranges[0]);
			majorTicksDouble = totalLength/NUM_MAJOR_TICKS;
			...
			rangeSlider.setValueAt(0, intmin);
			rangeSlider.setValueAt(1, intmax);
			titleValues = Double.toString((double)intmin) + "           " + title +
					"           " + Double.toString((double)intmax);
			rangeSlider.setBorder(BorderFactory.createTitledBorder(BorderFactory.createRaisedBevelBorder(),
					titleValues, TitledBorder.CENTER, TitledBorder.ABOVE_TOP));
		}
		else {
	...
	public void setPaintLabel(boolean paintLabel) {
		...
		this.valueAt0 = this.rangeSlider.getValueAt(0);
		this.valueAt1 = this.rangeSlider.getValueAt(1);
		...
		(int)(minorTicksDouble*divisor),                   // ~60
		divisor);
		rangeSlider.setValueAt(0, this.valueAt0);
		rangeSlider.setValueAt(1, this.valueAt1);
		titleValues = Double.toString((double)this.valueAt0) + "       " + title +
				"       " + Double.toString((double)this.valueAt1);
		rangeSlider.setBorder(BorderFactory.createTitledBorder(BorderFactory.createRaisedBevelBorder(),
					titleValues, TitledBorder.CENTER, TitledBorder.ABOVE_TOP));
    }
...
/*	public static void main (String[] args) {
		JFrame test = new JFrame("TEST OF ATTRIBUTE SLIDER");
		test.setSize(800, 800);
        //RangeSlider testRangeSliderDefaults = new RangeSlider();
        //test.getContentPane().add(testRangeSliderDefaults, BorderLayout.CENTER);
        /*
         RangeSlider testRangeSlider1 = new RangeSlider("tmax", MultiSlider.HORIZONTAL,										// alignment
         2,			//	numberOfThumbs
         0,			//	min
         255,										//	max
         50,			//major ticks
...
/*		double[] minMax =  {
		...
		RangeSlider testRangeSlider1 = new RangeSlider();
		Object[] rangesTitle = new Object[2];
		rangesTitle[0] = "Test";
		rangesTitle[1] = minMax;
        //testRangeSlider1.setTitle("TEST");
        ...
        //testRangeSlider1.setTabTitle("test");
		RangeSlider testRangeSlider2 = new RangeSlider();
		test.getContentPane().add(testRangeSlider1, BorderLayout.NORTH);
        //		test.getContentPane().add(testRangeSlider2, BorderLayout.CENTER);
