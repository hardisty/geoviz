...
public class Divseqellipseup extends SchemeObject{
  ...
  public Divseqellipseup(int vclass, int hclass, int maxlightness, int minlightness, int alpha, int e, int dx, int dy, int startinghue) {
    ...
    for(int i = 0; i < vclass; i ++){
      for(int j = 0; j < hclass; j ++){
        ...
        b = Math.abs(maxlightness) + (Math.abs(minlightness) - Math.abs(maxlightness))*i/(vclass - 1);
        a = b/Math.sqrt(1 - (double)e*e/10000.00);

        angle = 270 - alpha/2 + alpha*j/(hclass - 1);
        k = Math.tan(Math.toRadians(angle));

        y = (-1)*Math.abs(a*b*k)/Math.sqrt(b*b + a*a*k*k);
        x = y/k;
