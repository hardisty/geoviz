...
import geovista.jts.feature.AttributeType;
import geovista.jts.feature.Feature;
import geovista.jts.feature.FeatureCollection;
import geovista.jts.feature.FeatureSchema;
import geovista.jts.io.dbffile.DbfFieldDef;
import geovista.jts.io.dbffile.DbfFile;
import geovista.jts.io.dbffile.DbfFileWriter;
import geovista.jts.io.shapefile.Shapefile;
...
public class ShapefileWriter implements JUMPWriter {
            ...
            for (u = 0; u < fs.getAttributeCount(); u++) {
                ...
                if (columnType == AttributeType.INTEGER) {
                    ...
                    if (a == null) {
                        ...
                        DBFrow.add(dbfFile.dateParser.format((Date)a));
                    }                    
                } else if (columnType == AttributeType.STRING) {
                    Object a = feature.getAttribute(u);
  ...
  //XXX TODO un-suppress warnings and use factory
     ...
     *   All the geometry types will be the same type (i.e. all polygons) - or they will be set to<br>
     *     NULL geometries<br>
     *<br>
