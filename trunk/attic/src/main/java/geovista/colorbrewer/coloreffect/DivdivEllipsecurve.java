...
public class DivdivEllipsecurve extends SchemeObject {
  ...
  public DivdivEllipsecurve(int vclass, int hclass, int range, int a, int b, int shift_h, int shift_r, int shift_p) {
    ...
    for(int i = 0; i < vclass; i ++){
      for(int j = 0; j < hclass; j ++){
        ...
        L = b - Math.sqrt(b*b - b*b*(a - Math.sqrt(A*A + B*B))*(a - Math.sqrt(A*A + B*B))/(a*a));
