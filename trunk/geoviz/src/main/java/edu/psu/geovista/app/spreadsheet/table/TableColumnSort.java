...
public class TableColumnSort {
    ...
    private void createSortMap (int col){
        ...
        if (col==0){//creat sort map for entire table
            ...
            //int mcol=edu.psu.geovista.app.spreadsheet.formula.Cell.transColViewToModel(col);//Model index
            int mcol=col;
            if (mcol<size){//Vector.setElementAt(colsort,mcol); won't throw edu.psu.geovista.app.spreadsheet.exception
                sorts.setElementAt(colsort,mcol);
            }
            else{//Vector.setElementAt(colsort,mcol); will throw edu.psu.geovista.app.spreadsheet.exception, MUST fill with null element
    ...
    public Vector getColumnSort(int col){
         int size=sorts.size();
         if (col>=size){
         //Vector.setElementAt(colsort,mcol); will throw edu.psu.geovista.app.spreadsheet.exception,
    ...
    private void sortRows(int col, boolean ascending){
      ...
      System.out.println("********* edu.psu.geovista.app.spreadsheet.tools.Sort ->");
    ...
    public void sortColumns(int col, int[] cols,boolean ascending){
      ...
      if (sortType==SortRange.COLUMNS ){
         for (int k=0;k<cols.length ;k++){//do sort for each column
             //int mcol=edu.psu.geovista.app.spreadsheet.formula.Cell.transColViewToModel(cols[k]);
            int mcol=cols[k];
   ...
   /**   sort Range
     ...
     * @param   column  column index in table edu.psu.geovista.app.spreadsheet.model
     */
    ...
    public Point getCellPointAt(int vx, int vy){
        int mx,my;//edu.psu.geovista.app.spreadsheet.model x, edu.psu.geovista.app.spreadsheet.model y
        my=vy;
