...
/**
 * RadViz is a GUI component that provides an interactive RadViz plot for exploring
 ...
 * @version 1.0
 */
public class RadViz extends JPanel implements ActionListener, ClassificationResultListener,
                                              DataSetListener, MouseListener, 
                                              MouseMotionListener, IndicationListener, 
                                              ItemListener, SelectionListener, SubspaceListener,
                                              TableModelListener
...
{        
    ...
    private Color backgroundColor = Color.white, selectionColor = Color.magenta;
    ...
    protected transient boolean draggingLeftBtn = false, drawLegendTip = false, indicationOn = false;
    protected transient boolean leftBtnPressed = false, leftBtnPressedOnAttLbl = false;
    ...
    protected transient JDialog attSetWtDialog = null, colorSettingsDialog = null;
    ...
    protected void initPopupMenu() {
        ...
        popup.add(menuItem);
        popup.addSeparator();
        recenterItem = new JMenuItem(RECENTER_CMD,
            new ImageIcon(this.getClass().getResource("resources/home16.gif")));
        recenterItem.addActionListener(this);
        recenterItem.setVisible(false);
        popup.add(recenterItem);
        panItem = new JMenuItem(PAN_CMD,
            new ImageIcon(this.getClass().getResource("resources/pan16.gif")));
...
//    
     ...
     *
     * @param datasetForApp the dataset to be displayed
     **/
    public void setDataSet(DataSetForApps datasetForApp) {
        ...
        //Set color code attribute and update classification indices and class labels
        //if necessary
        ...
        setClassLabels(labels);
        int[] default_classification = new int[datasetForApp.getNumObservations()];
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            data[i] = (double[])help.clone();//new double[help.length];
            //System.arraycopy(help, 0, data[i], 0, help.length);
            /*if (attNames[i].equalsIgnoreCase("RCALLAWhF9901")) {
                for (int j = 0; j < data[i].length; j++) {
                    logger.finest(data[i][j]);
                }
            }*/
        }
        for (int i = 0; i < data.length; i++) {
    ...
    /**
     * Returns names of numeric attributes of the dataset displayed in RadViz GUI.
     *
     ...
     *
     * @param attWeights the attribute weights
     */
     ...
     *
     * @param backgroundColor the background color of RadViz GUI
     */
     ...
     *
     * @param classification the classification indices
     */
     ...
     *
     * @param colorCodeAtt the color code attribute
     */
     ...
     *
     * @param colors the array of distinct colors
     */
    ...
    /**
     * Returns the color scheme of the array of distinct colors used to color code
     * the data.
     *
    ...
    /**
     * Sets the color scheme of the array of distinct colors used to color code the 
     * data.
     *
     * @param colorScheme the color scheme of the array of distinct colors
     */
     ...
     *
     * @param indicationId the index of the indicated data point
     */
    ...
    public void setIndicationToolTip(int indicationId) {
        if (indicationId >= 0) {
            ...
            for (int i = 0; i < visibleAttIdx.length; i++) {
                String help_str = (attNames[visibleAttIdx[i]] + " = " + data[visibleAttIdx[i]][indicationId] + ";");
     ...
     *
     * @param selection the indices of the selected data points
     */
    ...
    /**
     * Returns the color used to highlight the selected data points in RadViz GUI.
     *
     ...
     *
     * @param enabled a boolean representing the desired status
     */
    private void setPopupItems(boolean enabled) {
        MenuElement[] items = popup.getSubElements();
        for (int i = 0; i < items.length; i++) {
            if (items[i] == panItem || items[i] == zoomInMenu ||
                items[i] == zoomOutMenu)
            {
                if (!enabled || getVisibleAttIdx().length > 2) {
                    items[i].getComponent().setEnabled(enabled);
                }
            }
            else {
                items[i].getComponent().setEnabled(enabled);
    ...
    /**
     * Returns the indices of those attributes that will be displayed in RadViz GUI.
     *
    ...
    /**
     * Sets the indices of those attributes that will be displayed in RadViz GUI.
     *
     * @param visibleAttIdx the indices of the visible attributes
     */
    ...
    /**
     * Resets the screen coordinates of attribute labels displayed in RadViz GUI.
     */
    ...
    /**
     * Updates the classification indices and class labels based on the colorCodeAtt, 
     * an attribute by whose values we will assign the data records into groups
     * represented by distinct colors and class labels. Thus it is possible for 
     * us to color code the data points not only by the results of supervised or
     * unsupervised classification tasks but also by any categorical attribute.
     *
     * @param colorCodeAtt the attribute used to color code the dataset
     */
        ...
        if (objs[index] instanceof String[]) {
            ...
            for (int i = 0; i < ia.length; i++) {
                ...
                if (classHash.get(tempClassLbl) == null) {
                    ...
                    indices[i] = Integer.parseInt((String)classHash.get(tempClassLbl));
                }
            }
        }
            ...
            labels = new Vector();
            labels.add(new String("All"));
            indices = new int[datasetForApp.getNumObservations()];
            java.util.Arrays.fill(indices, 0);
            if (!getColorCodeAtt().equalsIgnoreCase(CLR_CODE_ATT_DEFAULT)) {
                setColorCodeAtt(CLR_CODE_ATT_DEFAULT);
            }
            */
        }
     ...
     *
     * @param xpos the x screen coordinate of the current mouse position
     * @param ypos the y screen coordinate of the current mouse position
        ...
        if (visibleAttIdx != null) {
            if (visibleAttIdx.length == 2) {
                for (int i = 0; i < ux.length; i++) {
                    e2d = new Ellipse2D.Double(ux[i] - pointSize, uy[i] - pointSize,
            ...
            if (visibleAttIdx.length == 2) {
                ...
                for (int i = 0; i < ux.length; i++) {
                    ...
                    //if (draggingPath.contains(dst)) {
                      logger.finest("d-selected"+dst.getX()+"//"+dst.getY());
     ...
     * 
     * @param dstr the array of double values to be normalized
     */
    ...
    /**
     * Helper function to draw the indication "tooltip" on the given graphics object.
     *
    ...
    private void drawIndicationTip(Graphics2D g2, int x, int y, String tipText) {
        ...
        if (this.getWidth() < (topX + toolTipSize.width)) {
            topX = this.getWidth() - toolTipSize.width - 1;
        }
        if (this.getHeight() < (topY + toolTipSize.height)) {
            topY = this.getHeight() - toolTipSize.height - 1;
        }
        if (topX < 1) {
            topX = 1;
        }
        ...
        g2.drawRect(topX - 1, topY - 1, toolTipSize.width + 2, toolTipSize.height + 2);
        ...
        if (tipText != null) {
            ...
            for (int i = 0; i < nRows; i++) {
                g2.drawString(st.nextToken(), topX + 5, topY + (i + 1) * textHeight);
            }
        }
    }
    
    /**
     * Helper function to draw a "tooltip" like legend on the given graphics object.
     *
     * @param g2 The Graphics2D Object to draw on
     * @text the textual labels for the classes in the legend
     * @param x The x coordinate of the upper left corner
     * @param y The y coordinate of the upper left corner
     * @param colors The color representing the classes in the legend
     */
    private void drawLegendTip(Graphics2D g2, int x, int y, List text, Color[] colors) {
        ...
        //Make sure the legend tip doesn't extend beyond the window bounds
        if (this.getWidth() < (topX + width)) {
            topX = this.getWidth() - width - 1;
        }
        if (this.getHeight() < (topY + height)) {
            topY = this.getHeight() - height - 1;
        }
        if (topX < 1) {
            topX = 1;
        }
        ...
        for (int i = 0; i < numClass; i++) {
            g2.setColor(bkup_clr);
            if (isBivariate) {
                g2.drawString(lines[i * 2], topX + 3 * pad + ovalW, 
                              topY + (i + 1) * pad + (2 * i + 1) * lineHeight / 2);
                g2.drawString(lines[i * 2 + 1], topX + 3 * pad + ovalW, 
                              topY + (i + 1) * (pad + lineHeight));
            }
            else {
                g2.drawString(lines[i], topX + 3 * pad + ovalW,
                              topY + (i + 1) * pad + (3 * i + 2) * lineHeight / 3);
            }
            g2.setColor(colors[i]);
            g2.fillOval(topX + pad, topY + (i + 1) * pad + i * lineHeight + 5, ovalW, ovalH);
        }
    }
     ...
     *
     * @param g the Graphics2D reference to be used for drawing
     * @param w the width of this RadViz panel
     * @param h the height of this RadViz panel
     */
    protected void drawRadviz(Graphics2D g, int w, int h) {
        ...
        AffineTransform defaultTransform = (AffineTransform)g.getTransform().clone();
        ...
        for (int i = 0; i < numAtt; i++) {
            ...
            float weight = (float)Math.round((float)attWeights[visibleAttIdx[i]] * 100)
                           / 100;
        ...
        /*if (radvizBox.getCurrentPoint() != null) {
            radvizBox.reset();
         }
         radvizBox.moveTo((float)sx[0], (float)sy[0]);
         for (int i = 1; i < numAtt; i++) {
             radvizBox.lineTo((float)sx[i], (float)sy[i]);
         }
         radvizBox.lineTo((float)sx[0], (float)sy[0]);
         radvizBox.closePath();
        ...
        radvizBox.setFrame(ul_dst.getX(), ul_dst.getY(), 2 * radius, 2 * radius);
        ...
        try {
            ...
            g.setClip(g.getTransform().createInverse().createTransformedShape(radvizBox));
        } catch (NoninvertibleTransformException nte) {
            System.err.println("RadViz.drawRadviz(): " + nte.getMessage());
            nte.printStackTrace();
        }
        Color clr_bkup = g.getColor();
        Color clr_bkup_alpha = new Color(clr_bkup.getRed(), clr_bkup.getGreen(),
                                         clr_bkup.getBlue(), alpha);
        ...
        for (int i = 0; i < numData; i++) {
            ...
            for (int j = 0; j < numAtt; j++) {
                sumj += data[visibleAttIdx[j]][i] * attWeights[visibleAttIdx[j]];
            ...
            for (int j = 0; j < numAtt; j++) {
                double wij = 0.0;
                if (sumj > 0) {
                    wij = data[visibleAttIdx[j]][i] * attWeights[visibleAttIdx[j]] 
                          / sumj;
                }
                ux[i] += wij * sx[j];
                uy[i] += wij * sy[j];
    ...
    protected void drawScatter2D(Graphics2D g, int w, int h) {
        ...
        double[] xValCopy = (double[])xVal.clone();
        ...
        double[] yValCopy = (double[])yVal.clone();
        ...
        for (int i = 1; i < 5; i++) {
            xTicks[i] = String.valueOf((float)Math.round((xFloor + i * xtdiff) * 100)
                                       / 100);
            yTicks[i] = String.valueOf((float)Math.round((yFloor + i * ytdiff) * 100)
                                       / 100);
        ...
        int ymin = lpad;
        if (edgeLength > (ymax - ymin)) edgeLength = ymax - ymin;
        ...
        visibleAttLblBnds[0] = new Rectangle2D.Double(
                       (double)((xmin + xmax - metrics.stringWidth(xName)) / 2),
                       (double)(ymax + textHeight + 2 * spad),
                       (double)metrics.stringWidth(xName), (double)textHeight);
        g.rotate(Math.PI / (-2));
        g.drawString(yName, -(ymin + ymax + metrics.stringWidth(yName)) / 2,
                     xmin - yTextWidth - 3 * spad);
        g.rotate(Math.PI / 2);
        visibleAttLblBnds[1] = new Rectangle2D.Double((double)(xmin - textHeight - yTextWidth - 3 * spad),
                       (double)((ymin + ymax - metrics.stringWidth(yName)) / 2),
                       (double)textHeight, (double)metrics.stringWidth(yName));
        ...
        /*
         ...
         radvizBox.lineTo((float)xmin, (float)ymin);
         radvizBox.closePath();
         g.draw(radvizBox);
         */
        ...
        for (int i = 0; i < 6; i++) {
            ...
            g.drawString(yTicks[i], xmin - 2 * spad - metrics.stringWidth(yTicks[i]),
                         ytemp + textHeight / 3);
        ...
        //Draws the data points
        double xValDiff = (double)(xCeil - xFloor);
        double yValDiff = (double)(yCeil - yFloor);
        Color clr_bkup = g.getColor();
        Color clr_bkup_alpha = new Color(clr_bkup.getRed(), clr_bkup.getGreen(),
                                         clr_bkup.getBlue(), alpha);
        if (missingData.size() > 0) {
    ...
    /**
     * Draws the plot layer which contains either a multidimenional RadViz plot or a
     * 2-dimensional scatter plot. This layer is implemented as a BufferedImage with
     * the same size as the RadViz GUI. On top of this layer is the selection layer 
     * where all selections and indications are displayed.
     */
    ...
    /**
     * Updates the plot layer display. If desired also updates classification indices
     * and class labels, the selection list and the indication index.
     *
     * @param updateCC a boolean value deciding if an update is desired
     * @param updateIndication a boolean value deciding if an update is desired
     * @param updateSelection a boolean value deciding if an update is desired
     */
    protected void updatePlotLayer(final boolean updateCC,
                                   final boolean updateIndication,
...
//        worker.execute();
    ...
    /**
     * Overrides the default component rendering method. This method first renders
     * the BufferedImage containing the plot layer, and then renders the selection
     * layer above it.
     *
     * @param g the Graphics reference to be used for rendering this RadViz GUI
     */
    public void paintComponent(Graphics g) {
        super.paintComponent(g);
        //Draws the plot layer
        if (ri == null || ri.getWidth() != this.getWidth() || 
            ri.getHeight() != this.getHeight()) {
            makePlotLayerImg();
        }
            ...
            if (visibleAttIdx != null) {
                if (visibleAttIdx.length == 2) {
                    for (int i = 0; i < selected.length; i++) {
                        g2d.fill(new Ellipse2D.Double(ux[selected[i]] - pointSize,
                                                      uy[selected[i]] - pointSize,
                                                      2 * pointSize + 1,
                                                      2 * pointSize + 1));
                    }
                }
            ...
            if (visibleAttIdx != null) {
                if (visibleAttIdx.length == 2) {
                    g2d.fill(new Ellipse2D.Double(ux[indicated] - pointSize,
                                                  uy[indicated] - pointSize,
                                                  2 * pointSize + 1,
                                                  2 * pointSize + 1));
                }
                else if (visibleAttIdx.length > 2) {
                    Ellipse2D e2d = new Ellipse2D.Double(
                                                    ux[indicated] - pointSize,
                                                    uy[indicated] - pointSize,
                                                    2 * pointSize + 1,
                                                    2 * pointSize + 1);
                    Shape s = radvizTransform.createTransformedShape(e2d);
     ...
     */
    /* Commented by Gary - seems unnecessary
    public void fireDataSetChanged(Object[] dataSet) {
        Object[] listeners = listenerList.getListenerList();
        DataSetEvent e = null;
     ...
     *
     * @param indicator the new indication index 
     */
     ...
     *
     * @param newSelection the new list of selected indices
     */
     ...
     *
     * @param selection the new list of attributes for a new subspace
     */
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getClickCount() == 2 && e.getButton() == MouseEvent.BUTTON1) {
            //left button double clicked, opens the attSetWtDialog for changing the
            //weight of the clicked attribute or animating the effects of changing
            //its weight
            if (!panModeOn && visibleAttLblBnds != null && visibleAttLblBnds.length > 2) {
                String[] attNames = getAttNamesNumeric();
                for (int i=0; i < visibleAttLblBnds.length; i++) {
                    if (visibleAttLblBnds[i].contains(e.getX(),e.getY())) {
                        ...
                        if (attSetWtDialog == null) {
                            ...
                            attSetWtDialog = new JDialog(dummyFrame, title, true);
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1) {
            ...
            //Left button down, be prepared for the user to drag either a selection
                ...
                if (visibleAttLblBnds != null) {
                    for (int i=0; i < visibleAttLblBnds.length; i++) {
                        if (visibleAttLblBnds[i].contains(e.getX(),e.getY())) {
                            ...
                            srcAttIdx = i;
                            attDraggingIcon.setRect(
                                e.getX() - visibleAttLblBnds[i].getWidth()/2.0, 
                                e.getY() - visibleAttLblBnds[i].getHeight()/2.0,
                    ...
                    if (!leftBtnPressedOnAttLbl) {
                        ...
                        if (getSelection() != null) { //Clear current selection
                            setSelection(null);
                            repaint();
    ...
    public void mouseReleased(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1) {
            if (panModeOn) {
                if (radvizBox != null &&
                    radvizBox.contains(startX, startY)) {
        ...
        if (!panModeOn) {
            if (leftBtnPressed) {
                ...
                if (leftBtnPressedOnAttLbl) {
                    ...
                    attDraggingIcon.setRect(e.getX() - attDraggingIcon.getWidth()/2.0,
                                            e.getY() - attDraggingIcon.getHeight()/2.0,
                                            attDraggingIcon.getWidth(),
                                            attDraggingIcon.getHeight());
                }
                else {
    ...
    public void mouseMoved(MouseEvent e) {
        ...
        if (!panModeOn) {
            if (visibleAttLblBnds != null && visibleAttLblBnds.length > 0) {
                for (int i = 0; i < visibleAttLblBnds.length; i++) {
                    if (visibleAttLblBnds[i] != null && visibleAttLblBnds[i].contains(currX, currY)) {
                        showHandCursor = true;
                        break;
                    }
                }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == timer) {
            ...
            switch (animationType) {
                case ANIMATION_ND: //Animating the effects of changing multiple weights
                    ++currFrameId;
                ...
                case ANIMATION_GTND: //Multi-D grand tour animation
                    ...
                    if (gtCurrRound == 1) {
                        attWeights[visibleAttIdx[gtCurrAtt]] = 0.0 +
                            (double)(numFrames - currFrameId) / (numFrames - 1);
                        makePlotLayerImg();
                        repaint();
                        if (currFrameId == numFrames) {
                            ++gtCurrRound;
            ...
            if (colorSettingsDialog == null) {
                ...
                colorSettingsDialog.getContentPane().setLayout(new BorderLayout());
            ...
            if (attSelDialog == null) {
                ...
                attSelDialog = new JDialog(dummyFrame, SET_VISIBLE_ATT_CMD, true);
                ...
                attSelDialog.getContentPane().add(attSelPanel, BorderLayout.CENTER);
            }
            String[] attNames = getAttNamesNumeric();
            ...
            attWeightsBkup  = (double[])attWeights.clone();
            String[] attNames = getAttNamesNumeric();
            ...
            String s = (String)JOptionPane.showInputDialog(dummyFrame, "Attribute",
                                                           SET_CLR_ATT_CMD,
        ...
        else if (e.getActionCommand().equalsIgnoreCase("AttSelectPanel.CANCEL")) {
            //User discarded the change of the selection of visible attributes
            attSelDialog.setVisible(false);
        }
        else if (e.getActionCommand().equalsIgnoreCase("AttSetWeightPanel.OK")) {
        ...
        else if (e.getActionCommand().equalsIgnoreCase("AttSetWeightPanel.CANCEL")) {
        ...
        else if (e.getActionCommand().equalsIgnoreCase("AttSetWeightPanel.WeightIsChanging")) {
            //User is constantly changing the weight of the selected attribute to
        ...
        else if (e.getActionCommand().equalsIgnoreCase("AttWeightsAnimationPanel.OK")) {
            //User chose to replace current attribute weights with the set of end
            //attribute weights as defined in the dialog
            animationDialog.setVisible(false);
            double[] newWeights = (double[])animationPanel.getEndWeights().clone();
        ...
        else if (e.getActionCommand().equalsIgnoreCase("AttWeightsAnimationPanel.CANCEL")) {
            ...
            if (!nochange) {
                attWeights = (double[])attWeightsBkup.clone();
                makePlotLayerImg();
            }
            repaint();
        }
        else if (e.getActionCommand().equalsIgnoreCase("AttWeightsAnimationPanel.ANIMATE")) {
            ...
            if (numFrames > 0) {
                ...
                if (!nodiff) {
                    attWeights = (double[])startWts.clone();
                    makePlotLayerImg();
                    repaint();
        ...
        else if (e.getActionCommand().equalsIgnoreCase("AttWeightsAnimationPanel.STOP")) {
            if (timer.isRunning()) {
                timer.stop();
            }
            animationType = -1;
            ...
            if (visibleAttIdx.length == 2) {
                ...
                numSeconds = Math.round((float)numSeconds / (visibleAttIdx.length
                                        * 2));
                numFrames = fps * numSeconds;
                delay = (1000 * numSeconds) / numFrames;
                gtCurrAtt = 0;
        ...
        else if (e.getActionCommand().equalsIgnoreCase("GrandTourPanel.CANCEL")) {
        ...
        else if (e.getActionCommand().equalsIgnoreCase("RadvizColorSettingsGUI.OK")) {
            //User accepted the color settings changes
            RadvizColorSettingsGUI csPanel = (RadvizColorSettingsGUI)e.getSource();
            ...
            if (csPanel.getAlpha() != alpha || csPanel.getPointSize() != pointSize) {
                if (!updatePlot) {
                    updatePlot = true;
                }
        ...
        else if (e.getActionCommand().equalsIgnoreCase("RadvizColorSettingsGUI.CANCEL")) {
    ...
    public void itemStateChanged(ItemEvent e) {
        ...
        if (e.getStateChange() == ItemEvent.SELECTED) {
            if (source == indicationOnItem) {
                ...
                String zoomScaleText = source.getSelectedObjects()[0].toString();
            ...
            if (source == indicationOnItem) {
                this.indicationOn = false;
                if (this.getIndicationId() >= 0) {
                    repaint();
                }
            }
            else if (source == showLegendItem) {
     ...
     *
     * @param e a ClassificationResultEvent
     */
	...
	public void tableChanged(TableModelEvent e) {
		this.setDataSet(this.datasetForApp);
