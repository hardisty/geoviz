...
public class BasicParallelDisplayUI extends ParallelDisplayUI implements MouseListener, MouseMotionListener {
	...
	protected final static Logger logger = Logger.getLogger(BasicParallelDisplayUI.class.getName());
    //GeneralPath[] rPaths;
    int numDimensions;
    int numRecords;
    ...
    public void paint(Graphics g, JComponent c){
        ...
        if (renderThread == null){
            ...
            renderThread.setStyle(new BasicStroke(.5f), new Color(0.0f, 0.0f, 0.0f, 0.7f));
            renderThread.start();
        }
        ...
        if (brushThread == null){
            ...
            brushThread.setStyle(new BasicStroke(1.0f), new Color(0.0f, 0.0f, 0.0f, 0.8f));
	...
	g2.setRenderingHints(qualityHints);
        ...
        if (comp.deepRepaint){
            ...
            if(this.conditioning == null){
              this.conditioning = new int[numRecords];
            }
            ...
            if (comp.getBrushedCount() == 0) brushImg = null;
            renderAll();
        ...
        if (paintHoverNative || paintHoverComp){
            ...
            GeneralPath rPath = this.assemblePath(paintHoverRecord,0,numDimensions-1,comp);
    ...
    private GeneralPath assemblePath(int num, int startAxis, int stopAxis, ParallelDisplay comp){
      GeneralPath rPath = new GeneralPath();
      float val = this.getYValue(num,startAxis,comp);
     ...
     void drawRecord(Graphics2D g2, ParallelDisplay comp, int num, int startAxis, int stopAxis){
       if (this.conditioning.length != this.numRecords){
         this.conditioning = new int[numRecords];
       }
       if (this.conditioning[num] < 0){
          return;
        }
...
//        } else {
         rPath = this.assemblePath(num,startAxis,stopAxis, comp);
//        }
        if (this.colors != null) {
          g2.setColor(this.colors[num]);
        }
        if (inBrush){
          g2.setColor(Color.blue);
        }
    ...
    void drawBrushedRecord(Graphics2D g2, ParallelDisplay comp, int num, int startAxis, int stopAxis){
      if (this.conditioning[num] < 0){
        return;
        }
      GeneralPath rPath = this.assemblePath(num,startAxis,stopAxis,comp);
      //if (this.colors != null) {
    ...
    public void createBrushImage(ParallelDisplay comp) {
      if (brushImg == null) {
                    //brushImg = (BufferedImage)comp.createImage(comp.getWidth(), comp.getHeight());
            ...
            case ParallelDisplay.REORDER:
                if (activeAxis != -1){
                    ...
                    if (activeAxis != num) {
                        ...
                        logger.finest("setting repaint axes: " + (Math.min(num,activeAxis) - 1) + ", " + (Math.max(num,activeAxis) + 1));
                    ...
                    for (int i=0; i<comp.getNumRecords(); i++){
                        if ((ids.length > 0) && (i == ids[id])){
                            ...
                            if (e.isShiftDown() && e.isAltDown()){
                                ...
                                if (brushVal > 1.0f) brushVal = 1.0f;
     ...
     * @return The recordnumber of the first record found passing through the given point.
     */
    public int[] getAllRecordsByCoordinates(Rectangle2D hitBox, ParallelDisplay comp){
