...
import edu.psu.geovista.app.spreadsheet.functions.Function;
import edu.psu.geovista.app.spreadsheet.functions.FunctionManager;
...
public class Formula {
    ...
    // the raw edu.psu.geovista.app.spreadsheet.formula string
    ...
    // whether this edu.psu.geovista.app.spreadsheet.formula needs recalculation
    private boolean needsRecalc;

    /**
     * edu.psu.geovista.app.spreadsheet.formula.Formula contructor.
     *
     * This is used to construct a edu.psu.geovista.app.spreadsheet.formula.Formula object without any
     * parsing process.
     *
     * @param input the edu.psu.geovista.app.spreadsheet.formula string
     * @param row the current row where the edu.psu.geovista.app.spreadsheet.formula is stored
     * @param col the current column where the forluma is stored
     * @param e a edu.psu.geovista.app.spreadsheet.exception.ParserException
     */
     ...
     * @param row the current row where the edu.psu.geovista.app.spreadsheet.formula is stored
     * @param col the current column where the forluma is stored
     * @edu.psu.geovista.app.spreadsheet.exception edu.psu.geovista.app.spreadsheet.exception.ParserException
     * @see #toPostfix
     */
    public Formula(Cell owner, String input, int row, int col) throws ParserException {
            ...
            try {
                ...
                // tokenize and convert the edu.psu.geovista.app.spreadsheet.formula to postfix form (a list of edu.psu.geovista.app.spreadsheet.formula.Node)
                nodes= tokenize(formulaString);
                //Debug.println("edu.psu.geovista.app.spreadsheet.formula.Formula() nodes: "+nodes);
                ...
                Debug.println("edu.psu.geovista.app.spreadsheet.formula.Formula constructor: "+e);
                throwError(e);
            }
    }
    /*
     ...
     ********************************************************************/
     ...
     private void evalueExpression(LinkedList nodes){
        if (this.isBad() ){//for invalid edu.psu.geovista.app.spreadsheet.formula not passing
            return;
        }
...
/*int x=(int)address.getX() ;
                ...
                sb.append(addrs);
                    ...
                    if (start.getType() ==Node.REL_ADDR ){
                        ...
                        //throw edu.psu.geovista.app.spreadsheet.exception: unable to handle the expression
                    }
	...
	// postfix was set to null when there was any error
        // in processing the edu.psu.geovista.app.spreadsheet.formula string
        return postfix == null;
    }
   ...
   /**
    ...
    /**
     * Tokenize the edu.psu.geovista.app.spreadsheet.formula string into a list of Nodes.
     *
     * @param input the input string to tokenize
     * @edu.psu.geovista.app.spreadsheet.exception edu.psu.geovista.app.spreadsheet.exception.ParserException
     *
     * @see edu.psu.geovista.app.spreadsheet.formula.Node
     */
            ...
            while (cur < input.length()) {
             ...
             try {
                ...
                if (Character.isLetter(c)) {
                    // 1. edu.psu.geovista.app.spreadsheet.functions.Function or Relative Address
...
/*||
                     (lastType == edu.psu.geovista.app.spreadsheet.formula.Node.DEFAULT ||
                      lastType == edu.psu.geovista.app.spreadsheet.formula.Node.LPAREN || lastType == edu.psu.geovista.app.spreadsheet.formula.Node.COMMA) &&
                      (c == '+' || c == '-')) */
                    // Numbers
                    while (cur < input.length() &&
                       (Character.isDigit(input.charAt(cur)) ||
                ...
                if (lastToken != null &&
                    ...
                    lastToken.isPending()) { //Range
                                if (node.isType(Node.REL_ADDR) || node.isType(Node.ABS_ADDR)) {
                                    ...
                                    node = range;
                                    //			util.Debug.println("edu.psu.geovista.app.spreadsheet.formula.Node: "+node);
                                } else
                                    throwError("#ADDR?");
                }

                 //		edu.psu.geovista.app.spreadsheet.util.Debug.println("Add: "+node);
                ...
                throwError(e);
                    ...
                    e.printStackTrace() ;
                 ///edu.psu.geovista.app.spreadsheet.util.Debug.println(e.toString());
      } //try
     ...
     * list of nodes (that is, each parameter can be a edu.psu.geovista.app.spreadsheet.formula).
     *
     * The basic rules are:
     * <ol>
	...
	try {
		...
		if (node.isType(Node.FUNCTION)) {
		    ...
		    if (param.isType(Node.LPAREN)) {
                ...
                //			edu.psu.geovista.app.spreadsheet.util.Debug.println("exp is "+list);
     ...
     *
     * @return a HashSet of edu.psu.geovista.app.spreadsheet.formula.CellPoint that the current cell references
     */
    ...
    *
    * @param table the TableModel object
    * @param postfix the edu.psu.geovista.app.spreadsheet.formula in postfix form
    ...
    */
    public  Number evaluate(LinkedList postfix) throws ParserException,NoReferenceException {
            ...
            try {
                        ...
                        if (o==null){ //If refrenced cell is null, show 0
                            ...
                            if( o.equals("#REF!")){
                                ...
                                if (numOfNode==1&&!it.hasNext() ){
                                    ...
                                    //If the edu.psu.geovista.app.spreadsheet.formula contain ONLY the cell, just throw the String to the referencing cell
                                    // which is setValue() as the string
                                    throw new ParserException((String)o);
                                }
                                else{
                                    ////If the edu.psu.geovista.app.spreadsheet.formula contain more than the cell, throw  "#VALUE!"
                ...
                // ("Wrong format of edu.psu.geovista.app.spreadsheet.formula: too many operators");
            }catch (ParserException e) {
                throw e;
                ...
                e.printStackTrace() ;
                //edu.psu.geovista.app.spreadsheet.util.Debug.println(e);
            }
    ...
    private void checkReference(Cell cell) throws ParserException{
                        if(this.owners.contains(cell)){
                            this.setBad() ;//stop any further evaluation on the edu.psu.geovista.app.spreadsheet.formula
...
//        }
    ...
    // The following are just simple edu.psu.geovista.app.spreadsheet.functions
    ...
    /**
     * Label the bad cells and throw edu.psu.geovista.app.spreadsheet.exception.ParserException.
     * error is saved so next time it won't re-evaluate again:
     * it directly throws the same edu.psu.geovista.app.spreadsheet.exception.
     *
	...
	// test code
        //	System.err.println("Marking edu.psu.geovista.app.spreadsheet.formula "+formulaString+" as bad");
        postfix = null;
