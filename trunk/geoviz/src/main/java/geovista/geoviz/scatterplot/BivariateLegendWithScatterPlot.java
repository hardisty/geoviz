...
public class BivariateLegendWithScatterPlot extends ScatterPlotWithBackground {
	...
	// transient protected Color[][] classColors;//commented out hardisty 1 may
	// 07
	// transient protected ScatterPlotWithBackground detailSP;//commented out
	// hardisty 1 may 07
		...
		for (int i = 0; i < lenX; i++) {
			...
			if (Math.abs(mouseX1 - v) <= 5) {
				...
				if ((mouseX2 - mouseX1) > 0 && (i + 1) < lenX) {
					...
					} else if (i != 0 && mouseX2 > xBoundariesInt[i - 1]
							&& mouseX2 < xBoundariesInt[i + 1]) {// avoid
						// over
						// data
						// range
						xBoundariesInt[i] = mouseX2;
						xBoundaries[i] = getValueFromScreenValue(mouseX2,
								xScale, plotOriginX, xBoundaries[0]);
					...
					if (Math.abs(mouseX2 - v1) <= 3) {
						...
						fireActionPerformed(BivariateLegendWithScatterPlot.COMMAND_BOUNDARIES_NUMBERCHANGED);
					} else if (i != lenX - 1 && mouseX2 > xBoundariesInt[i - 1]
							&& mouseX2 < xBoundariesInt[i + 1]) {
						xBoundariesInt[i] = mouseX2;
						xBoundaries[i] = getValueFromScreenValue(mouseX2,
								xScale, plotOriginX, xBoundaries[0]);
						fireActionPerformed(BivariateLegendWithScatterPlot.COMMAND_BOUNDARIES_MOVED);
				...
				if (i == 0) {
					...
					if ((mouseX2 - mouseX1) > 0 && (mouseX2 < v1)) {
						...
						xBoundaries[1] = getValueFromScreenValue(mouseX2,
								xScale, plotOriginX, xBoundaries[0]);
			...
			if (Math.abs(mouseY1 - v) <= 5) {
				...
				if ((mouseY2 - mouseY1) > 0 && (i + 1) < lenY) {
					...
					if (Math.abs(mouseY2 - v1) <= 5) {
						...
						fireActionPerformed(BivariateLegendWithScatterPlot.COMMAND_BOUNDARIES_NUMBERCHANGED);
					} else if (i != 0 && mouseY2 < yBoundariesInt[i - 1]
							&& mouseY2 > yBoundariesInt[i + 1]) {
						yBoundariesInt[i] = mouseY2;
						yBoundaries[i] = getValueFromScreenValue(mouseY2,
								yScale, plotOriginY, yBoundaries[0]);
						fireActionPerformed(BivariateLegendWithScatterPlot.COMMAND_BOUNDARIES_MOVED);
					}
					...
					if (Math.abs(mouseY2 - v1) <= 3) {
						...
						fireActionPerformed(BivariateLegendWithScatterPlot.COMMAND_BOUNDARIES_NUMBERCHANGED);
					} else if (i != lenY - 1 && mouseY2 < yBoundariesInt[i - 1]
							&& mouseY2 > yBoundariesInt[i + 1]) {
						yBoundariesInt[i] = mouseY2;
						yBoundaries[i] = getValueFromScreenValue(mouseY2,
								yScale, plotOriginY, yBoundaries[0]);
				...
				if (i == 0) {
					...
					if ((mouseY2 - mouseY1) < 0 && (mouseY2 > v1)) {
						...
						yBoundaries[1] = getValueFromScreenValue(mouseY2,
								yScale, plotOriginY, yBoundaries[0]);
			...
			if ((mousePos[0] - xBoundariesInt[0]) < 50) {
				for (int i = 0; i < yBoundariesInt.length - 1; i++) {
					...
					if ((mousePos[1] - v1) < -5 && (v2 - mousePos[1]) < -5) {
						...
						yBoundaries[i + 1] = getValueFromScreenValue(
								mousePos[1], yScale, plotOriginY,
								yBoundaries[0]);
			...
			if ((yBoundariesInt[0] - mousePos[1]) < 50) {
				...
				for (int i = 0; i < xBoundariesInt.length - 1; i++) {
					...
					if ((mousePos[0] - v1) > 5 && (v2 - mousePos[0]) > 5) {
						...
						xBoundaries[i + 1] = getValueFromScreenValue(
								mousePos[0], xScale, plotOriginX,
								xBoundaries[0]);
