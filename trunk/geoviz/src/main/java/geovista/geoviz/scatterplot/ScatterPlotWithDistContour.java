...
public class ScatterPlotWithDistContour extends ScatterPlot {
	...
	protected void setupDataforDisplay() {
		...
		eigenAngle = Math.PI
				- Math.atan(eigenDisplay.getEigenVectors()[1][1]
						/ eigenDisplay.getEigenVectors()[0][1]);
		// contour mean
		meanXInt = this.getValueScreen(meanX, xScale, plotOriginX,
				xAxisExtents[0]);
		meanYInt = this.getValueScreen(meanY, yScale, plotOriginY,
				yAxisExtents[0]);
		// the end points for two vectors at one std distance
		ellipseMajorXInt = (int) (meanXInt - stdInt[1]
				* eigenDisplay.getEigenVectors()[0][1]);
		ellipseMajorYInt = (int) (meanYInt - stdInt[1]
				* eigenDisplay.getEigenVectors()[1][1]);
		ellipseMinorXInt = (int) (meanXInt - stdInt[0]
				* eigenDisplay.getEigenVectors()[0][0]);
		ellipseMinorYInt = (int) (meanYInt - stdInt[0]
				* eigenDisplay.getEigenVectors()[1][0]);
		ellipseMajorX2Int = (int) (meanXInt + stdInt[1]
				* eigenDisplay.getEigenVectors()[0][1]);
		ellipseMajorY2Int = (int) (meanYInt + stdInt[1]
				* eigenDisplay.getEigenVectors()[1][1]);
		ellipseMinorX2Int = (int) (meanXInt + stdInt[0]
				* eigenDisplay.getEigenVectors()[0][0]);
		ellipseMinorY2Int = (int) (meanYInt + stdInt[0]
				* eigenDisplay.getEigenVectors()[1][0]);
	...
	private void drawEigenVectorsCross(Graphics g) {
		...
		g.drawLine(ellipseMajorXInt, ellipseMajorYInt, ellipseMajorX2Int,
				ellipseMajorY2Int);
		g.setColor(Color.gray);
		g.drawLine(ellipseMinorXInt, ellipseMinorYInt, ellipseMinorX2Int,
				ellipseMinorY2Int);
	}

	public Shape toShape() {
		Ellipse2D.Double e = new Ellipse2D.Double(meanXInt - stdEigenVector1,
				meanYInt - stdEigenVector2, stdEigenVector1 * 2,
				stdEigenVector2 * 2);
		AffineTransform at = AffineTransform.getRotateInstance(eigenAngle,
				meanXInt, meanYInt);
