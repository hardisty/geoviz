...
public class ExcentricLabels extends MouseAdapter implements Comparator,
    ...
    public void setComponent(JComponent c) {
        if (c instanceof ExcentricLabelClient) {
            ...
            throw new RuntimeException("Invalid component type for ExcentricLabels");
    }
    ...
    public void paint(Graphics2D graphics, Rectangle2D bounds) {
        ...
        for (int i = 0; i < labelCount; i++) {
            int hit = hits[i];
            String lab = client.getObservationLabel(hit);// added fah 18 march 2003
    ...
    protected void computeItemPositions(Graphics2D graphics,
        ...
        for (int i = 0; i < labelCount; i++) {
            //Rectangle2D rect = visualization.getShapeAt(hits.get(i)).getBounds2D();
          int hit = hits[i];
    ...
    public void mouseMoved(MouseEvent e) {
        if (isVisible()) {
            if (dist2(centerX - e.getX(), centerY - e.getY()) > threshold * threshold) {
                setVisible(false);
                insideTimer.restart();
            }
            client.repaint();
