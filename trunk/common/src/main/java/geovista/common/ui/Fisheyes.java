...
public class Fisheyes {
    ...
    public Rectangle2D getBounds() {
        if (bounds == null) {
            bounds = new Rectangle2D.Float(focusX - focusRadius,
                                           focusY - focusRadius,
    ...
    public Shape transform(Shape s) {
        ...
        float       prev_tx = 0;
        float       prev_ty = 0;
        for (PathIterator iter = s.getPathIterator(null); !iter.isDone();
                 iter.next()) {
            ...
            case PathIterator.SEG_LINETO: {
                ...
                float y = coords[1];
                transform(coords, 1);
                subdivide(prev_x, prev_y, prev_tx, prev_ty,
                          x, y, coords[0], coords[1], p);
                prev_x = x;
                prev_y = y;
                prev_tx = coords[0];
                prev_ty = coords[1];
            ...
            case PathIterator.SEG_CLOSE: {
                subdivide(prev_x, prev_y, prev_tx, prev_ty,
                          first_x, first_y, first_tx, first_ty, p);
                break;
            }
            }
        }
     ...
     * @param tx1 Y coordinate of first point
     * @param ty1 transformed Y coordinate of first point
     * @param x2 X coordinate of second point
     * @param y2 Y coordinate of second point
     * @param tx2 Y coordinate of second point
     * @param ty2 transformed Y coordinate of second point
     * @param p GeneralPath to fill
     */
    public void subdivide(float x1, float y1, float tx1, float ty1,
    ...
    private QuadCurve2D.Float leftQuad = new QuadCurve2D.Float();
    private QuadCurve2D.Float rightQuad = new QuadCurve2D.Float();
    /**
     ...
     * @param tx3 Y coordinate of third point
     * @param ty3 transformed Y coordinate of third point
     * @param p GeneralPath to fill
     */
    public void subdivide(float x1, float y1, float tx1, float ty1,
                           float x2, float y2, float tx2, float ty2,
        ...
        subdivide(x1, y1, tx1, ty1,
                  leftQuad.ctrlx, leftQuad.ctrly, txLeft, tyLeft,
                  leftQuad.x2, leftQuad.y2, txm, tym,
                  p);
        p.lineTo(txm, tym);
        ...
        subdivide(rightQuad.x1, rightQuad.y1, txm, tym,
                  rightQuad.ctrlx, rightQuad.ctrly, txRight, tyRight,
                  x3, y3, tx3, ty3,
                  p);
    }

    private CubicCurve2D.Float leftCubic = new CubicCurve2D.Float();
    private CubicCurve2D.Float rightCubic = new CubicCurve2D.Float();
    /**
     * Subdivide a quad segment.
     *
     * @param x1 X coordinate of first point
     ...
     * @param tx3 Y coordinate of fourth point
     * @param ty3 transformed Y coordinate of fourth point
     * @param p GeneralPath to fill
     */
    public void subdivide(float x1, float y1, float tx1, float ty1,
                           float x2, float y2, float tx2, float ty2,
        ...
        subdivide(x1, y1, tx1, ty1,
                  leftCubic.ctrlx1, leftCubic.ctrly1, tctrlx1, tctrly1,
                  leftCubic.ctrlx2, leftCubic.ctrly2, tctrlx2, tctrly2,
                  leftCubic.x2, leftCubic.y2, txm, tym,
                  p);
        p.lineTo(txm, tym);
        ...
        subdivide(rightCubic.x1, rightCubic.y1, txm, tym,
                  rightCubic.ctrlx1, rightCubic.ctrly1, tctrlx1, tctrly1,
                  rightCubic.ctrlx2, rightCubic.ctrly2, tctrlx2, tctrly2,
                  x3, y3, tx3, ty3,
                  p);
    }
