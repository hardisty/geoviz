...
/**
 * Title: ClassifierModifiedQuantiles
 * Description: Modified Quantile classification method
        ...
        for (int i = 0; i < numClasses; i++) {
          if (i == 0){
            boundaries[i] = dataWithIndex[0][0];
                  ...
                  boundaries[i] = dataWithIndex[(int)(nPerClass*((double)i - 1.5))][0];
          } else {
			...
			for (int i = 0; i < numClasses; i++) {
			    ...
			    if (i < (numClasses - 1)){
					if (i == 0){
						...
						endElm = (int)(numInEachClass*((double)i - 0.5)) - 1;
					} else {
