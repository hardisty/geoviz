...
public abstract class SpatialStatistics {
	...
	/**
	  */// 
	  ...
	  public static double[] calculateMoranScores(double[] zData, SpatialWeights sw) {
	...
	public static double[] calculateRandomMoranScores(double[] zData, SpatialWeights sw) {
	      double[] moranData;
	...
	public static List<double[]> findMonteValues (double[] data, int iterations, SpatialWeights sw){
	...
	public static double[] findPValues(double[] zData, double[] moranValues, int numTries, SpatialWeights sw){
		 
	  	 //if this approach uses too much memory, we could do it in iterations, using the sqrt of 
		  ...
		  for (int obs = 0; obs < zData.length; obs++){
			  pVals[obs] = DescriptiveStatistics.percentAbove(monteVals.get(obs), zData[obs]);
		  }
