...
public class Seqseqgray extends SchemeObject{
  ...
  public Seqseqgray(int vclass, int hclass, int maxlightness, int minlightness, int alpha, int dx, int dy, int startinghue){
    ...
    double a = 0;
    double b = 0;
    double x = (maxlightness - minlightness)/(4*Math.cos(Math.toRadians(alpha))*hclass);
    double y = (maxlightness - minlightness)/(4*Math.cos(Math.toRadians(alpha))*vclass);
    ...
    for(int i = 0; i < vclass; i ++){
      for(int j = 0; j < hclass; j ++){
        ...
        a = Math.cos(Math.toRadians(startinghue))*Math.sin(Math.toRadians(alpha))*((1 + 2*i)*x - (1 + 2*j)*y);

        b = Math.sin(Math.toRadians(startinghue))*Math.sin(Math.toRadians(alpha))*((1 + 2*i)*x - (1 + 2*j)*y);
