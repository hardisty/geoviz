/* -------------------------------------------------------------------
 Java source file for the class RmiListener
 Original Authors: Frank Hardisty and Linna Li
 $Author: hardisty $
 $Id: RmiListener.java,v 1.2 2006/02/27 19:28:41 hardisty Exp $
...
public class RmiListener
    extends UnicastRemoteObject implements RemoteMessageReceiver, RemoteSelectionListener,
  ...
  protected final static Logger logger = Logger.getLogger(RmiListener.class.getName());
  ...
  public RmiListener(int port) throws RemoteException {
    ...
    try {
      ...
      if (!RmiListener.boundObjectToRegistry) {
        if (RemoteCollaboration.logger.isLoggable(Level.FINEST)) {
          logger.finest("registry already started, will not bind RmiListener class");
        }
        //create registry
        ...
        if (RemoteCollaboration.logger.isLoggable(Level.FINEST)) {
          logger.finest("registry already started, looking up registry");
        }
        //find registry
        registry = LocateRegistry.getRegistry(this.thisPort);
      }
  ...
  public static Object connectToRemoteJVM(String ip) {
    ...
    Preferences gvPrefs = Preferences.userNodeForPackage(RemoteCollaboration.class);
    ...
    try {
      ...
      if (RemoteCollaboration.logger.isLoggable(Level.FINEST)) {
        logger.finest("Remote obj.getClass().getName() = " + obj.getClass().getName());
      }
  ...
  public static void startRegistry() {
    if (RmiListener.startedSystemRegistry) {
      if (RemoteCollaboration.logger.isLoggable(Level.FINEST)) {
        logger.finest("system java rmi registry already started, will not restart");
      }
    ...
    try {
      ...
      Runtime.getRuntime().exec("rmiregistry"); //start the rmi registry on the local
      //machine. The registry exists outside this jvm.
      //There needs to be a pause of a second or so after this local command is called, before
  ...
  public void extentChanged(String source, double[] extent) throws RemoteException {
    ...
    //(x,y,w,h)
    Rectangle2D.Double extentRect = new Rectangle2D.Double(extent[0], extent[1], extent[2],
        extent[3]);
    this.client.remoteSpatialExtentChanged(source, extentRect);
  }
  ...
  public void receiveMessage(String name, String msg) throws RemoteException {
    ...
    this.client.remoteMessageReceived(name, msg);
  }
  ...
  public void selectionChanged(String source, int[] selection) throws RemoteException {
    ...
    this.client.remoteSelectionChanged(source, selection);
  }
  ...
  public void subspaceChanged(String source, int[] subspace) throws RemoteException {
    ...
    //we got this selection from somewhere (maybe another JVM)
    //send it along to registered listeners in this JVM
    this.client.remoteSubspaceChanged(source, subspace);
  }
