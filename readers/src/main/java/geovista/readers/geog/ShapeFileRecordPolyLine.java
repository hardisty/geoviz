...
*
* Java source file for the class ShapeFileRecordPolyLine    (1)
*
...
*
* $Id: ShapeFileRecordPolyLine.java,v 1.3 2005/09/15 14:54:06 hardisty Exp $
*
* To Do:
* ___
*
...
/**
* Holds a polyline's data
*
* @see ShapeFile.java
* @version $Id: ShapeFileRecordPolyLine.java,v 1.3 2005/09/15 14:54:06 hardisty Exp $
...
public class ShapeFileRecordPolyLine {
	...
	private double[][] points = {{0}}; // all points in polyline

	private int dataSize; // in shorts

	public ShapeFileRecordPolyLine() {
	}

	public ShapeFileRecordPolyLine(ShapeFileDataInputStream dis, boolean littleEndian) throws IOException {
		dataSize = 0;
		if(littleEndian) // littleEndian should always be true
			dataSize = parseData(dis, dataSize);
	}

	public ShapeFileRecordPolyLine(ShapeFileDataInputStream dis, int theDataSize) throws IOException {
		...
		if(shapeType != ShapeFile.SHAPE_TYPE_POLYLINE)
			throw new IOException("ShapeFileRecordPolyLine(), Unexpected shapeType = " + shapeType);
	...
	private int parseData(ShapeFileDataInputStream dis, int dataRead) throws IOException {
		...
		for(int i = 0; i < numParts; i++) {
			...
			if((nextIndex < 0) || (nextIndex >= numPoints))
				throw new IOException("ShapeFileRecordPolyLine(), Invalid ringIndex = " + nextIndex);
			parts[i] = nextIndex;
		}
	...
	public void write(ShapeFileDataOutputStream dos, boolean writeShapeType) throws IOException {
		if(writeShapeType) {
			dos.writeIntLE(ShapeFile.SHAPE_TYPE_POLYLINE);
		}
	...
	public String toString() {
		String res = new String("NumPolyLines=" + numParts + " Origin X,Y=" + box[XMIN] + ", " + box[YMIN] );
		return res;
	}
}
