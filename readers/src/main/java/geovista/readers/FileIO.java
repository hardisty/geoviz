	...
	public FileIO(String filename, String filemode) throws InvalidFileModeException, FileIOException, IOException {
		...
		if (filename == null) {		// use stdin/stdout if filename is null
			if(this.mode==READ_MODE){
				this.ir = new geovista.readers.Reader(System.in);
				this.pw = null;
			} else {
				this.ir = null;
			...
			if (this.mode == READ_MODE) { // unzip or ungzip
				...
				case ZIP:
					...
					this.ir = new geovista.readers.Reader(zf.getInputStream(target));
					break;
				case GZIP:
					GZIPInputStream gzis = new GZIPInputStream(new FileInputStream(filename));
					this.ir = new geovista.readers.Reader(gzis);
					break;
				}
				this.pw = null;
			} else {	// zip or gizp
			...
			if (this.mode == READ_MODE) {
				this.ir = new geovista.readers.Reader(new FileInputStream(filename));
				this.pw = null;
			} else {
				this.ir = null;
