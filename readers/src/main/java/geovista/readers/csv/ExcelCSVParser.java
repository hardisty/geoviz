 ...
 * </pre>
 * <P>
 * The CSV that Excel outputs differs from the <a
 * href="http://ostermiller.org/utils/CSVLexer.html">standard</a> in several
 * respects:
 * <ul>
...
public class ExcelCSVParser implements CSVParse {
	 ...
	 * @since ostermillerutils 1.00.00
	 */
	private final ExcelCSVLexer lexer;
	 ...
	 */
	public ExcelCSVParser(InputStream in, char delimiter) {
		inStream = in;
	 ...
	 * @since ostermillerutils 1.02.24
	 */
	public ExcelCSVParser(Reader in, char delimiter) {
		inReader = in;
		lexer = new ExcelCSVLexer(in);
		changeDelimiter(delimiter);
	}
	 ...
	 * <p>
	 * If the line has already been partially read, only the values that have
	 * not already been read will be included.
	 * 
	 ...
	 * <p>
	 * If the file has already been partially read, only the values that have
	 * not already been read will be included.
	 * <p>
	 ...
	 * 
	 * @param s
	 *            string with comma delimited data to parse.
	 * @return parsed data.
	 * 
	 ...
	 * 
	 * @param s
	 *            string with delimited data to parse.
	 * @param delimiter
	 *            record separator
	 * @return parsed data.
	 * @throws BadDelimiterException
	...
	public static String[][] parse(String s, char delimiter) {
		try {
			return (new ExcelCSVParser(new StringReader(s), delimiter))
					.getAllValues();
		} catch (IOException x) {
			return null;
		}
	 ...
	 * 
	 * @param in
	 *            Reader with comma delimited data to parse.
	 * @return parsed data.
	 * @throws IOException
	 *             if an error occurs while reading.
	 * 
	 ...
	 * 
	 * @param in
	 *            Reader with delimited data to parse.
	 * @param delimiter
	 *            record separator
	 * @return parsed data.
	 * @throws BadDelimiterException
