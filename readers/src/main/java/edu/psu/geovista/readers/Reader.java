 ...
 *
 * Java source file for the class Reader
 *
 * Copyright (c), 2002, Masahiro Takatsuka & GeoVISTA Center
 * All Rights Reserved.
 *
 * This is a copy of jh9gpz.io.MtReader.java originally written by
 * Masahiro Takatsuka
 *
 * Original Author: Masahiro Takatsuka
 * $Author: xpdai $
 *
 * $Date: 2003/07/11 19:08:20 $
 *
...
public final class Reader extends java.io.Reader {
	...
	public Reader(InputStream in, String delim, boolean returnTokens) {
		...
		this.delimiters = (delim == null) ? "" : delim; // 2000-Jan-12	0.1.1
    ...
    public final int read(char cbuf[], int off, int len) throws IOException {
		synchronized (lock) {
			ensureOpen();
			return this.br.read(cbuf, off, len);
		}
    }
	...
	*/
	...
	/**
	* Reads the specified numbers of characters from the specified Reader
	* and returns as a String.
	*/
	public final static char[] readChars(java.io.Reader reader, int len) throws IOException {
	...
	* and returns as a String.
	*/
	public final static int readChars(char[] chars, java.io.Reader reader, int len) throws IOException {
		int i = 0;
		for (i = 0; i < len; i++) {
	...
	* and returns as a String.
	*/
	public final static String read(java.io.Reader reader, int len) throws IOException {
		StringBuffer sbuf = new StringBuffer();
		boolean flag = false;
		for (int i = 0; i < len; i++) {
			char c = (char) reader.read();
    ...
    /**
     * Tell whether this stream is ready to be read.  An InputReader is
    ...
    private void skipDelimiters() throws IOException {
		int ch;
		if (!this.retTokens) {
			this.br.mark(1);			// set a mark.
			while ((ch = this.br.read()) != -1 &&
				   this.delimiters.indexOf((char)ch) >= 0) {
				this.br.mark(1);
			}
			if (ch == -1) {		// has reached EOF.
				this.eof = true;
			} else {
				this.br.reset();			// go back one char.
			}
		}
    }
	...
	public final String readToken() throws IOException {
		skipDelimiters();
		if (this.eof) {				// has reached EOF during skipDelimiters()
			return null;
		}
		...
		if (this.retTokens && (buf.length() == 0) &&
			(this.delimiters.indexOf((char)ch) >= 0)){
			buf.append((char)ch);
		} else {
			this.br.reset();			// go back one char.
	...
	public final char readChar() throws IOException {
		...
		int ch;
		this.br.mark(1);			// set a mark.
		if ((ch = this.br.read()) != -1 ) {
			if (!this.retTokens && this.delimiters.indexOf((char)ch) >= 0) {
				this.br.reset();
			} else {
				this.br.mark(1);
			}
		} else {
			this.eof = true;			// has reached EOF.
		}
