/* -------------------------------------------------------------------
 ...
 Java source file for the class ListeningBeanGUI
 Copyright (c), 2002, GeoVISTA Center
 All Rights Reserved.
 Original Author: Frank Hardisty
 $Author: hardisty $
 $Id: ListeningBeanGUI.java,v 1.2 2003/04/25 17:58:13 hardisty Exp $
...
public class ListeningBeanGUI extends JPanel implements ActionListener {
  ...
  */
  public ListeningBeanGUI(ListeningBean lBean, CoordinationManager cm) {
    ...
    ArrayList[] lists = ListeningBeanGUI.seperateFiringMethods(firingMethods);

    //each list should now contain a different interface
    listenersGUIPanel = new JPanel();
    listenersGUIPanel.setLayout(
        new BoxLayout(listenersGUIPanel, BoxLayout.Y_AXIS));
    this.listenGUI = new ListeningInterfaceGUI[firingMethods.length];

    for (int i = 0; i < lists.length; i++) {
      FiringMethod[] meths = ListeningBeanGUI.listToFMethod(lists[i]);
      ListeningInterfaceGUI liGUI = new ListeningInterfaceGUI(lBean, meths, cm);
      this.listenGUI[i] = liGUI;
      this.listenersGUIPanel.add(liGUI);
    }
  ...
  public static ArrayList[] seperateFiringMethods(FiringMethod[] meths) {
    ...
    for (int i = 1; i < meths.length; i++) {
      if (meths[i].getListeningInterface() != meths[i - 1].getListeningInterface()) {
        numUnique++;
      }
    }
