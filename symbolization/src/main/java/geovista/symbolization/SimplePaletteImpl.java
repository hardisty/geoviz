...
public class SimplePaletteImpl implements Palette{
	protected final static Logger logger = Logger.getLogger(SimplePaletteImpl.class.getName());
    private Color low = Color.white;
    private Color high = Color.black;
    private Vector anchors = new Vector();
    private ColorRamp ramp = new ColorRamp();
    private String name;
    ...
    public Color[] getColors(int length) {
        ...
        while(i < anchors.size()){
            int position = recalculateAnchorPosition(((Integer) anchors.get(i)).intValue(), length);
    ...
    private int recalculateAnchorPosition(int oldPosition, int newSize){
        return Math.round(((float)oldPosition / (float)noOfInputColors) * newSize);
    }
    ...
    public int getRecommendedMaxLength() {
        //30 sounds good, doesn't it?  There's no theory behind the choice of this number.
        return 30;
    }
    ...
    public boolean isDivergent() {
        //Returns false even though it has no idea whether this is true or false.
        return false;
    }
