...
public class AffineTransformModifier {
  ...
  public static AffineTransform makeGeogAffineTransform(Rectangle2D source, Rectangle2D target,
                                              boolean mirror, boolean flipVertical){

    AffineTransform xForm = new AffineTransform();
    double sX, sY; //tX, tY for translation, sX, sY for scaling

         //we need to know two things: how much to scale, and how much to translate
         ...
         //tY = ((double)this.getHeight()) - (yMax -yMin);
          ...
          //remember that transforms are applied last to first, so scale then translate
          xForm = new AffineTransform();
          double rotation = Math.toRadians(180);//

          if (flipVertical) {
            xForm.translate((source.getWidth() * sFactor) + (target.getX() * 2),(source.getHeight() * sFactor) + (target.getY() * 2)); //fix problems generated by turning upside down
            xForm.rotate(rotation);//turn upside down
          }
          //step 5: if flipVertical, flip then fix

          if (mirror) {
            xForm.translate((source.getWidth() * sFactor) + (target.getX() * 2),0); //fix problem created by flipping mirror image
          ...
          //do nothing, i.e. no nudge factor necassary
            ...
            double newWidth = sFactor * (double)source.getWidth();
            double halfTargetWidth = (double)target.getWidth() / 2;
          ...
          if (origin.getY() > 2 + target.getY()) {
            AffineTransform xForm2 = new AffineTransform();
            xForm2.translate(0,-origin.getY());
            xForm.preConcatenate(xForm2);
          }
          */
  ...
  public static AffineTransform makeZoomingAffineTransform (Rectangle2D source, Rectangle2D target) {
    AffineTransform xForm = new AffineTransform();
